...
pub fn write_grid_csv<W: Write>(writer: &mut W, result: &SimulationResult) -> io::Result<()> {
    ...
    for gp in &result.energy_grid {
        let pos = gp.position;
        let bands = gp.energy;
        // Compute broadband from SANITISED bands so a single NaN doesn't NaN-poison the broadband column.
        let broadband = bands.iter().copied().map(safe_f32).sum::<f32>() / bands.len() as f32;
        writeln!(
            writer,
            "{},{},{},{},{},{},{},{},{},{}",
            safe_f32(pos.x),
...
pub fn write_text_report<W: Write>(
    ...
    let max_broadband = result.max_energy.iter().copied().fold(0.0_f32, f32::max);
    writeln!(writer, "Max broadband:    {:.4e}", max_broadband)?;
    writeln!(writer)?;
...
mod tests {
    ...
    fn small_result() -> SimulationResult {
        ...
        r.energy_grid.push(GridPoint {
            ...
            energy: [0.5, 1.0, 1.5, 2.0, 2.5, 3.0],
        });
        r.energy_grid.push(GridPoint {
            // Include a non-finite band value to confirm safe_f32 sanitises it.
            position: Vec3::new(3.1, 2.71, 1.41),
            energy: [f32::NAN, 0.0, 0.1, 0.2, 0.3, 0.4],
        });
        r
    }
    ...
    fn report_result_with_listener() -> SimulationResult {
        ...
        for (i, slot) in spl_bands.iter_mut().enumerate() {
            *slot = Some(80.0 - i as f32 * 3.0);
        }
    ...
    fn csv_broadband_column() {
        ...
        for ((idx, line), gp) in s.lines().enumerate().skip(1).zip(r.energy_grid.iter()) {
            let fields: Vec<f32> = line
                ...
                .collect();
            // Final column (index 9) is broadband (mean of sanitised band values).
            let csv_bb = fields[9];
            let expected_bb = safe_f32(
                gp.energy.iter().copied().map(safe_f32).sum::<f32>() / gp.energy.len() as f32,
            );
            assert!(
                (csv_bb - expected_bb).abs() < 1e-6,
                "row {idx}: broadband column ({csv_bb}) should equal mean of sanitised bands ({expected_bb})"
            );
            // Also confirm the per-band columns match the sanitised band array.
            for b in 0..BAND_COUNT {
                let expected = safe_f32(gp.energy[b]);
